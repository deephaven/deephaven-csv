...
public class Parsers {
    ...
    public static final List<Parser<?>> DEFAULT =
            List.of(BOOLEAN, INT, LONG, DOUBLE, DATETIME, CHAR, STRING);

    /**
     * The above plus BYTE. The TIMESTAMP_* parsers are never included by default, because they look like ints/longs.
     */
    public static final List<Parser<?>> COMPLETE =
            List.of(BOOLEAN, BYTE, SHORT, INT, LONG, DOUBLE, DATETIME, CHAR, STRING);

    /** Like COMPLETE but with FLOAT_FAST rather than DOUBLE. */
    public static final List<Parser<?>> COMPLETE_FLOAT =
            List.of(BOOLEAN, BYTE, SHORT, INT, LONG, FLOAT_FAST, DATETIME, CHAR, STRING);

    /** Minimal */
    public static final List<Parser<?>> MINIMAL = List.of(BOOLEAN, LONG, DOUBLE, DATETIME, STRING);

    /** Strings only. */
    public static final List<Parser<?>> STRINGS = List.of(STRING);

    /** DateTime, Double, Boolean, Char, String, and timestamp (seconds). */
    public static final List<Parser<?>> STANDARD_TIMES =
            someOtherParsersAnd(Parsers.TIMESTAMP_SECONDS);

    /** DateTime, Double, Boolean, Char, String, and timestamp (milliseconds). */
    public static final List<Parser<?>> STANDARD_MILLITIMES =
            someOtherParsersAnd(Parsers.TIMESTAMP_MILLIS);

    /** DateTime, Double, Boolean, Char, String, and timestamp (microseconds). */
    public static final List<Parser<?>> STANDARD_MICROTIMES =
            someOtherParsersAnd(Parsers.TIMESTAMP_MICROS);

    /** DateTime, Double, Boolean, Char, String, and timestamp (nanoseconds). */
    public static final List<Parser<?>> STANDARD_NANOTIMES =
            someOtherParsersAnd(Parsers.TIMESTAMP_NANOS);
